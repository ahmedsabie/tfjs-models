...
export async function setupDatGui(urlParams) {
  ...
  const modelController = modelFolder.add(
      params.STATE, 'model', Object.values(faceLandmarksDetection.SupportedModels));
...
// The MediaPipeFaceMesh model config folder contains options for MediaPipeFaceMesh config
// settings.
function addMediaPipeFaceMeshControllers(modelConfigFolder) {
  ...
  const predictIrisesController =
    modelConfigFolder.add(params.STATE.modelConfig, 'predictIrises');
    predictIrisesController.onChange(_ => {
  params.STATE.isModelChanged = true;
  });

  const maxFacesController = modelConfigFolder.add(
    params.STATE.modelConfig, 'maxFaces', 1, 10).step(1);
